...
(wk.register
  {:<c-j> [(fn []
   ...
   :<c-space> {1 "coc#refresh()" 2 "Atualizar sugestões do autocomplete" :expr true}}
  ...
  {:<c-n> ["<cmd>BufferLineCycleNext<CR>" "Próximo buffer"]
   ...
   "]"
     {"]" ["<cmd>call search('^\\w\\+\\s:\\s' 'w')<CR>" "Pular para a próxima função Elm"]
      ...
      :w ["<cmd>lua require'functions'['jump-next-word']()<CR>" "Próxima palavra destacada"]}
   "["
     {"[" ["<cmd>call search('^\\w\\+\\s:\\s' 'bW')<CR>" "Pular para a função Elm anterior"]
      ...
      :w ["<cmd>lua require'functions'['jump-previous-word']()<CR>" "Palavra destacada anterior"]}}
...
(wk.register
  {"," ["mpA,<Esc>`p" "\",\" no fim da linha"]
   ...
   :e
     ...
     :g ["<cmd>lua require'functions'['vim-grep']()<CR>" "Buscar com vimgrep"]
     :i ["<cmd>lua require'functions'['display-image'](vim.fn.expand('<cfile>'))<CR>" "Exibir imagem sob o cursor"]
     :q ["<cmd>qa<CR>" "Fechar"]}
   :g
     {:name "Git"
     ...
     :k ["<cmd>lua require'functions'['checkout-new-branch']()<CR>" "Criar branch e fazer checkout"]
     ...
     :y ["<cmd>lua require'functions'['lazygit-toggle']()<CR>" "Abrir lazygit"]}
     :h ["<cmd>split<CR> " "Dividir horizontalmente"]
     :i ["mpgg=G`p" "Indentar arquivo"]
     :l ["<cmd>lua require'functions'['toggle-location-list']()<CR>" "Alternar locationlist"]
     :m
       {:name "Markdown"
     ...
     :q ["<cmd>lua require'functions'['toggle-quickfix']()<CR>" "Alternar quickfix"]
     :s
       {
       :name "Sessão"
       :c ["<cmd>CloseSession<CR>" "Fechar (close)"]
       :d
        ["<cmd>lua require'functions'['command-with-args']('Delete session> ', 'default', 'customlist,xolox#session#complete_names', 'DeleteSession')<CR>"
         "Deletar"]
       :o
         ["<cmd>lua require'functions'['command-with-args']('Open session> ', 'default', 'customlist,xolox#session#complete_names', 'OpenSession')<CR>"
         "Abrir"]
       :s
...
(map! [i :nowait :expr] :<c-b> "coc#float#has_scroll() ? \"\\<c-r>=coc#float#scroll(0)\\<cr>\" : \"\\<Left>\"")
...
(fn show-documentation []
  (if (>= (vim.fn.index ["vim" "help"] (vim.opt.filetype:get)) 0)
    ...
    (vim.fn.execute (.. "!" (vim.opt.keywordprg:get) " " (vim.fn.expand "<cword>")))))
