...
(packer ; Suporte à fennel
        (use :udayvir-singh/tangerine.nvim) (use :udayvir-singh/hibiscus.nvim)
        ; Ícones
        (use :kyazdani42/nvim-web-devicons) ; Editorconfig
        (use :gpanders/editorconfig.nvim) ; Sessões
        (use :xolox/vim-misc)
        (use! :xolox/vim-session :requires :xolox/vim-misc) ; Buscador
        ...
        (use! :lukas-reineke/indent-blankline.nvim :config
              (setup :indent_blankline {:show_current_context true}))
        ; Auto-fechamento de delimitadores
        (use! :windwp/nvim-autopairs :config (setup :nvim-autopairs {}))
        ; Operação com delimitadores
        (use :tpope/vim-surround) ; Se mover melhor com o f/t
        (use :unblevable/quick-scope) ; Text-objects melhorados e com seek
        (use :wellle/targets.vim) ; Explorador de arquivos
        (use! :kyazdani42/nvim-tree.lua :requires :kyazdani42/nvim-web-devicons
              :config (setup :nvim-tree {:disable_netrw false}))
        ; Warper para comandos do git
        (use! :tpope/vim-fugitive :opt true :cmd
              [:G :Git :Gdiff :Gclog :Gwrite])
        (use! :pwntester/octo.nvim :requires
              [:nvim-lua/plenary.nvim
               :nvim-telescope/telescope.nvim
               :kyazdani42/nvim-web-devicons] :config
              (setup :octo {})) ; Biblioteca de snippets
        (use :rafamadriz/friendly-snippets) ; Status line
        (use! :nvim-lualine/lualine.nvim :requires
              {1 :kyazdani42/nvim-web-devicons :opt true})
        ; Guia de atalhos
        (use! :folke/which-key.nvim :config
              (setup :which-key {:plugins {:spelling {:enabled true}}}))
        ; Cliente REST
        (use! :diepm/vim-rest-console :opt true :ft :rest) ; LSP do Nvim
        (use! :neoclide/coc.nvim :branch :release) ; Alternador de terminal
        (use! :akinsho/nvim-toggleterm.lua :config
              (setup :toggleterm
                     {:shade_terminals false
                      :direction :horizontal
                      :insert_mappings false
                      :terminal_mappings false})) ; Buffers no topo
                                ...
                                :diagnostics_indicator (fn [_
                                                         ...
                                                         (let [diagnostics {:error ""
                                                           ...
                                                           (.. diagnostics.error
                                                               ...
                                                               diagnostics.info)))}}))
        ; Copiloto
        (use :github/copilot.vim) ; Correção do CursorHold
        (use :antoinemadec/FixCursorHold.nvim) ; Colorir hexademical de cores
        (use! :norcalli/nvim-colorizer.lua :config
              (setup :colorizer
                     {1 "*" :css {:hsl_fn true} :scss {:hsl_fn true}}))
        ; Dashboard
        (use :ChristianChiarulli/dashboard-nvim) ; Wildmenu melhorado
        (use :gelguy/wilder.nvim) ; Integração com Dash (MacOS)
