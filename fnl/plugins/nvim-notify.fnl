(local M {1 :rcarriga/nvim-notify :event :VeryLazy})
  ...
  (fn slide-from [direction]
    [(fn [state]
       (let [next-height (+ 3 state.message.height)
             next-row (stages-util.available_slot state.open_windows
                                                  next-height direction)]
         (when next-row
           {:relative :editor
            :anchor :NE
            :width 1
            ...
            :style :minimal})))
     (fn [state]
       {:width {1 state.message.width :frequency 2}
        :col [(vim.opt.columns:get)]})
     (fn []
